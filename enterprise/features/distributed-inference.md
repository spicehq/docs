...
{% hint style="info" %}
Multi-node distributed inference is a **Spice.ai Enterprise** feature and requires the `distributed` build. Single-node model serving — including `model_type: glm4` — is available in Spice.ai OSS. See [Hugging Face model provider](/building-blocks/model-providers/huggingface).
{% endhint %}

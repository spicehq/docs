...
A model capable of SQL generation must be configured in the project's spicepod. When the request omits `model`, Spice uses the single compatible model configured in the spicepod; if none or more than one is configured, the request fails and the model must be named explicitly.

## Generate and run SQL

<mark style="color:blue;">`POST`</mark> `https://<project-cname>.spiceai.io/v1/nsql`
...
<mark style="color:blue;">`GET`</mark> `https://<project-cname>.spiceai.io/v1/nsql/context`
...
{% hint style="warning" %}
This endpoint requires Spice runtime **v2.1.0 or later**. Projects pinned to an earlier version return `404`. See [Spice Runtime Versions](../portal/app-spicepod/spice-runtime-versions.md).
{% endhint %}
...
```bash
curl -H "X-API-Key: <project-api-key>" \
  -H "Accept: application/json" \
  "https://<project-cname>.spiceai.io/v1/nsql/context?include_sampling=true&sampling_limit=5"
```
...
| `functions`    | Available function groups, filtered to the project's query context.                       |
